...
if(isset($_POST['save']))
{
      ...
      $sql='delete from '.PREFIXE_BDD.'module_to_page where id_module="3"';
      mysql_query($sql);
  ...
  while($row=mysql_fetch_array($res))
  {
      ...
      {
          $sql='insert into '.PREFIXE_BDD.'module_to_page (id_page,id_module) values ("'.$_POST['id_page_'.$row['symbol']][$i].'","3")';
          mysql_query($sql);
      }
  }   
...
<em>Choisir les emplacements pour la galerie vidéos</em>
          ...
          <div style="margin-top:10px;">
            <select name="id_page_<?php echo $row['symbol']; ?>[]" multiple="multiple" size="10" style="width:100%;border:0px;">
                ...
                while($row_page=mysql_fetch_array($res_page))
                {
                    ...
                    $sql_test='select * from '.PREFIXE_BDD.'module_to_page where id_module="3" and id_page="'.$row_page['id_page'].'"';
                    ...
                    while($row_sous_page=mysql_fetch_array($res_sous_pages))
                    {
                      ...
                      $sql_test='select * from '.PREFIXE_BDD.'module_to_page where id_module="3" and id_page="'.$row_sous_page['id_page'].'"';
                      $res_test=mysql_query($sql_test);
                      if(mysql_num_rows($res_test)==1) echo ' selected '; 
