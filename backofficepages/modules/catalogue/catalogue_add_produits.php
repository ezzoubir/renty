...
{
    // maj du texte 
    $sql='update  '.PREFIXE_BDD.'catalogue_produits set texte="'.addslashes($_POST['presentation']).'",texte_suite="'.addslashes($_POST['tiny']).'",titre="'.addslashes($_POST['titre']).'",texte2="'.addslashes($_POST['texte2']).'" where id_produit="'.$id_produit.'"';
    mysql_query($sql);
    ...
    if(isset($_POST['bool']))
{
$sql='update  '.PREFIXE_BDD.'catalogue_produits set bool="1" where id_produit="'.$id_produit.'"';
mysql_query($sql);
}
else
{
$sql='update  '.PREFIXE_BDD.'catalogue_produits set bool="0" where id_produit="'.$id_produit.'"';
mysql_query($sql);
}   
}
else if(isset($_POST['save']))
{
      $sql='insert into '.PREFIXE_BDD.'catalogue_produits (texte,texte_suite,titre,id_categorie,texte2) values("'.addslashes($_POST['presentation']).'","'.addslashes($_POST['tiny']).'","'.addslashes($_POST['titre']).'","'.(int)$_GET['id_page'].'","'.addslashes($_POST['texte2']).'")';
      mysql_query($sql);
...
<b>Nom : </b><input type="text" name="titre" value="<?php if(isset($id_produit)) echo stripslashes($row['titre']); ?>"><br /><br />
<b>Date : </b>
<input type="text" name="presentation" value="<?php if(isset($id_produit)) echo stripslashes($row['texte']); ?>">
<br /><br />
<b>Lieu : </b>
<input type="text" name="texte2" value="<?php if(isset($id_produit)) echo stripslashes($row['texte2']); ?>">
<br /><br />
<?php
...
<b>Flyer :</b> <input type="file" name="image1">
<br /><br />
<?php
...
?>
<input type="checkbox" name="bool" <?php if(isset($row['bool']) && $row['bool']==1) echo ' checked '; ?>> Prévente
<br />
<br />
